...
class EvalMaxSAT : public VirtualMAXSAT {
    ...
    unsigned int _timeOutFastMinimize=120;
...
public:
    EvalMaxSAT(unsigned int nbMinimizeThread=4, vector<set<uint32_t>> succ={},  vector<set<uint32_t>> pred={}, uint32_t g_size=0,
               vector<uint32_t> initial_solution = {}) : nbMinimizeThread(nbMinimizeThread), solver(solver)
    {
        std::vector<Glucose::Solver *> dummy;
        std::vector<Glucose::Solver *> share;
        for(unsigned int i=0; i<nbMinimizeThread; i++) {
            solverForMinimize.push_back(new MonGlucose41(dummy));
            share.push_back(((MonGlucose41 *)solverForMinimize.back())->solver);
        }
#ifdef FIXRANDOM
        srand(1);
#endif
        share.push_back(NULL);
        solver = new MonGlucose41(share);

        degrees.resize(pred.size());
        ...
        ((MonGlucose41 *)solver)->solver->succ = move(succ);
        ((MonGlucose41 *)solver)->solver->pred = move(pred);
        ((MonGlucose41 *)solver)->solver->g_size = g_size;
        ((MonGlucose41 *)solver)->solver->initial_solution = move(initial_solution);
...
//                if (! degrees.empty()) {
//                    std::sort(forSolve.begin(), forSolve.end(), [&](auto &o1, auto &o2) {
//                        if (abs(o1) < degrees.size() && abs(o2) >= degrees.size())
//                            return true;
//                        else if (abs(o1) >= degrees.size() && abs(o2) < degrees.size())
//                            return false;
//                        else if (abs(o1) >= degrees.size() && abs(o2) >= degrees.size())
//                            return abs(o1) < abs(o2);
//                        return degrees[abs(o1)] < degrees[abs(o2)];
//                    });
//                }
                chronoLastSolve.tic();
